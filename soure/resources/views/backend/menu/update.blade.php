          ...
          <!-- Horizontal Form -->
           <div class="box box-primary">
            ...
            <form  method="post" action="{{ route('update_menu') }}"  >
              ...
              <input type="text" name="id" value="{{ $menu_item["id"] }}" style="display: none">
              <div class="box-body">
               	...
               	<div class="form-group">
	                ...
	                <select class="form-control select2" style="width: 100%;" name="id_parent">
                      ...
                      <?php if ($menu["id_parent"]==0):?>
                         <option value="{{$menu["id"]}}" {{ ($menu_item["id_parent"]==$menu["id"])?"selected":"" }}>{{$menu["name"]}}</option>
                          <?php foreach ($menus as $key => $menu_item):?>
                            <?php if ($menu_item["id_parent"] ==$menu["id"]):?>
                              <option value="{{$menu_item["id"]}}" {{ ($menu_item["id_parent"]==$menu_item["id"])?"selected":"" }}>|---{{$menu_item["name"]}}</option>
                            <?php endif;?>
                          <?php endforeach;?>
                      <?php endif;?>
                     <?php endforeach;?>
              ...
              <div class="form-group">   
                ...
                <div>
                   <input type="hidden" class="form-control"  placeholder="Tên menu" name="icon"  id="icon" value="{{ $menu_item["icon"]}}">
